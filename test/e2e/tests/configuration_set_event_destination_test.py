...
def get_event_destination(ses_client, configuration_set_name):
    try:
        return ses_client.describe_configuration_set(ConfigurationSetName=configuration_set_name, ConfigurationSetAttributeNames=['eventDestinations'])
...
def simple_event_destination(simple_configuration_set, ses_client) -> Tuple[k8s.CustomResourceReference, Dict]:
    ...
    assert deleted
    wait_until_deleted(partial(get_event_destination(ses_client, configuration_set_cr['spec']['name'])))
...
class TestEventDestination:
    def test_create_update_event_destination(self, simple_event_destination, ses_client):
        ...
        get_current_event_destination = partial(get_event_destination(ses_client, configuration_set_name))
